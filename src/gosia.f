...
C
C          (9 May 2019, N. Warr) gosia-20081208.18
C            - Use Lehmer random number generator
C            - Allow asymmetric errors (Pawel J. Napiorkowski)
C            - Use NIST absorption coefficients
C            - Correct order of magnitude for finite-recoil correction
C            - Correct coefficient for multipole-mixing ratios
C          (7 March 2019, N. Warr) gosia-20081208.17
C            - Add E0 parameter to Leuven efficiency
C          (5 October 2018, N. Warr) gosia-20081208.16
C            - Increase default INT parameter in CONT option to 1000.
C          (5 February 2018, N. Warr) gosia-20081208.15
...
C      ZV     - energy meshpoints
     ...
     &                IRAWEX(50)
      ...
      COMMON /EFCAL / ABC(8,10) , AKAVKA(50,9,200) , THICK(200,7)
     ...
     &                IBYP
      ...
      COMMON /ME2D  / EAMX(100,3) , NAMX , IAMX(100) , IAMY(100,2)
      REAL*8 TIMEL
      INTEGER*4 LIFCT
      COMMON /LIFE1 / LIFCT(50) , TIMEL(3,50)
      REAL*8 DEVD, DEVU
     ...
     &  .5D0, 1.D0, 1.5D0/
C     Absorption coefficients in units of 1/cm for Ge (NIST)
      DATA (tau1(k),k=1,10)/17.752D0, 10.768D0, 5.057D0, 2.954D0,
     &     1.3260D0, .8842D0, .6020D0, .4371D0, .3048D0, .2479D0/
...
C     MeV (NIST)
      DATA (tau2(k,1),k=1,10)/.9935D0, .7498D0, .5446D0, .4599D0,
     &      .3719D0, .3301D0, .2812D0, .2279D0, .1659D0, .1351D0/
      DATA (tau2(k,2),k=1,10)/.4210D0, .3944D0, .3623D0, .3407D0,
     &      .3031D0, .2765D0, .2399D0, .1961D0, .1431D0, .1165D0/
      DATA (tau2(k,3),k=1,10)/15.409D0, 9.488D0, 4.687D0, 2.927D0,
     &      1.546D0, 1.1496D0, .8654D0, .6626D0, .4720D0, .3845D0/
      DATA (tau2(k,4),k=1,10)/23.412D0, 14.273D0, 6.836D0, 4.108D0,
     &      1.986D0, 1.3969D0, 1.0026D0, .7493D0, .5287D0, .4303D0/
      DATA (tau2(k,5),k=1,10)/84.580D0, 51.701D0, 23.796D0, 13.183D0,
     &      4.838D0, 2.629D0, 1.3589D0, .8001D0, .5039D0, .4042D0/
      DATA (tau2(k,6),k=1,10)/95.205D0, 59.424D0, 12.632D1, 71.595D0,
     &      25.491D0, 12.652D0, 5.243D0, 2.251D0, 1.0932D0, .8287D0/
      DATA (tau2(k,7),k=1,10)/91.196D0, 56.927D0, 27.431D0, 62.926D0,
     &      22.850D0, 11.324D0, 4.572D0, 1.829D0, .8054D0, .5922D0/
      ...
      IF ( iobl.LT.1 ) THEN
         IF ( op2.NE.'GOSI' ) THEN
            ...
            DO jexp = 1 , NEXPT ! For each experiment
               ...
               ttttt = TREP(IEXP)
               dsig = DSIGS(IEXP)
               WRITE (22,99998) dsig, IEXP
99998          FORMAT (1X/////5X,"DSIG VALUE=",1E13.8,2X,'FOR EXP.',
     &         1I2///)
               IF ( op2.NE.'STAR' ) THEN
                  jmm = IEXP
                  IF ( IEXP.NE.1 ) THEN
         ...
         ELSE
            IF ( MOD(icount,IPRM(6)).EQ.0 ) THEN
               ...
               DO jlin = 1 , nlinn
                  ...
                  jst = MIN(jsa+9,MEMAX)
                  jpr = MIN(10,MEMAX-jsa+1)
                  WRITE (22,99006) (ipm(jin),jin=1,jpr)
                  WRITE (22,99007) (GRAD(jin),jin=jsa,jst)
               ENDDO
            ENDIF
         ENDIF
...
C      Chilo  - chi squared using logs
      ...
      REAL*8 DMIXE , DMIX
      INTEGER*4 IMIX , NDL
      COMMON /MIXD  / DMIXE(20,3) , DMIX(20) , IMIX(20) , NDL
      INTEGER*4 LNY , INTR , IPS1
      COMMON /LOGY  / LNY , INTR , IPS1
      dl = 0.

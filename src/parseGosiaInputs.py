...
def getMatrixElements(input_file):
  f = open(input_file)
  line = f.readline()
  while line != "ME\n":
    line = f.readline()
...
def findDof(input_file,yields_file):
  dofcount = 0
  f = open(input_file,'r')    
  line = f.readline()
...
def make_bst(bst_file,matrixElements):
  f = open(bst_file,'w')
  ...
  while line:
    ...
    if "!LT" in line: #Same as BR, but comment !LT for the lifetimes
      ...
      for i in range(nObs):
        ...
        splitline = line.split(',')
        beamExptMap.append((0,0,0))
        observables.append(float(splitline[1]))
        uncertainties.append(float(splitline[2]))
...
def getPOINobservables(output_file):
  computedObservables = []
